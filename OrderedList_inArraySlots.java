...
public class OrderedList_inArraySlots
    ...
    public OrderedList_inArraySlots
          ...
          // violates the directions for this hw
        for (int index = 0; index < unordered.size(); index ++) {
			int nextLowest = champIndex(unordered,index);
			int placeholder = unordered.get(index);
			unordered.set(index, unordered.get(nextLowest));
			unordered.set(nextLowest, placeholder);
		}
		list_iAS = unordered;
    }
     ...
     */
     private int champIndex(ArrayList<Integer> challengers, int startIndex) {
		int currentLow = challengers.get(startIndex);
		int indexAt = startIndex;
